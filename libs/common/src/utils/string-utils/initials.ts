...
type Options = { length: number; existing?: Record<string, string> } | number
...
const nonLetters = ' -\\/:-@\\[-`\\{-\\~'
// regex patterns
const uppercaseLettersOnlyPattern = /^[A-Z]+$/
const initialsInNamePattern = /\(([^)]+)\)/
const nameIsEmailPattern = /^[^\s]+@[^\s]+$/
const findDomainInEmailPattern = /@[^\s]+/
const findEmailPattern = /[\w._-]+@[\w.-]+[\w]/g

// match everything that is a "non letter" (see above)
// followed by all but a "non letter".
// Expl: "Jörg Jäger-Franke" => ["J", " J", "-F"]
const findFirstLettersOfWordsPattern = new RegExp(
  '(^|[' + nonLetters + '])[^' + nonLetters + ']',
  'g',
)
const findAllNonCharactersPattern = new RegExp('[' + nonLetters + ']', 'g')

/**
 * initials allows to be used with either a string or an array of strings
 */
function defaultInitials(nameOrNames?: NameOrNames, options?: Options) {
  if (!nameOrNames) return ''
  if (typeof nameOrNames === 'string')
    return initialsForSingleName(nameOrNames, normalize(options))
...
//
function addInitialsTo(nameOrNames: NameOrNames, options?: Options) {
  if (!nameOrNames) return ''
  if (typeof nameOrNames === 'string')
    return addInitialsToSingleName(nameOrNames, normalize(options))
...
function parse(nameOrNames, options) {
  if (!nameOrNames) return {}
  if (typeof nameOrNames === 'string')
    return parseSingleName(nameOrNames, normalize(options))
 ...
 */
function initialsForSingleName(name: string, options) {
  let matches
  let result
  const initials = findPreferredInitials(name, options)
  ...
  const possibleInitials = getPossibleInitialsForName(name)
  let option

  // then we return the first option that has the required length
  for (let i = 0; i < possibleInitials.length; i++) {
...
function initialsForMultipleNames(names: string[], options) {
  const optionsForNames = []
  let optionsCountForNames
  const map = {}
  const duplicatesMap = {}
  const initialsForNamesMap = {}
  let initials
  ...
  names.forEach(function (name) {
    ...
    possibleInitials = getPossibleInitialsForName(name).filter(function (
      initials,
    ) {
  ...
  // remove duplicates
  const keys = []
  for (const k in initialsForNamesMap) {
    keys.unshift(k)
  }
...
function addInitialsToSingleName(name, options) {
  const parts = parseSingleName(name, options)
  return format(parts)
}
...
function parseMultipleNames(names: string[], options) {
  const initialsArray = initialsForMultipleNames(names, options)
...
function combineAll(array: string[]) {
...
function getPossibleInitialsForWord(word: string) {
  const options = []
  while (word.length) {
 ...
 */
function normalize(options?: Options) {
