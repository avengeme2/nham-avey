...
export function getAliasFromResolveInfo(
  ...
  const asts: ReadonlyArray<FieldNode> = resolveInfo.fieldNodes
...
export function parseResolveInfo(
  ...
  const fieldNodes: ReadonlyArray<FieldNode> = resolveInfo.fieldNodes
  ...
  return asts.reduce((tree, selectionVal: SelectionNode, idx) => {
    ...
    if (skipField(resolveInfo, selectionVal)) {
      ...
      if (isReserved) {
        ...
        if (selectionSet && options.deep && isCompositeType(fieldGqlType)) {
